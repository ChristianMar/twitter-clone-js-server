...
const getUserPosts = (req, res, router) => {
  ...
  posts = _.filter(posts, (item) => parseInt(item.userId) === parseInt(userId));
...
const createPost = (req, res, router) => {
  const { userId, title, post, image, createdAt, tags, isBig } = req.body;
  ...
  const db = router.db;
  let allTags = db.get("tags").value();

  tags.split(",").forEach((tag) => {
    let selected = allTags.filter((item) => item === tag);
